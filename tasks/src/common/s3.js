...
export class Bucket {
    ...
    async downloadFileToFolder({ file, localPath }) {
        localPath = path.join(localPath, path.dirname(file));
        const output = path.join(localPath, path.basename(file));
        ...
        const downloadParams = { Bucket: this.bucket, Key: file };
        const command = new GetObjectCommand(downloadParams);
        let response = await this.client.send(command);
