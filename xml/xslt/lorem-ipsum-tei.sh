...
if [ $# -ne 1 ] ; then
    ...
    echo "The new TEI file will be created with a suffix '.new.xml'; check it before over-writing the original"
else
    node /srv/tasks/node_modules/xslt3 -t -xsl:"/srv/tasks/src/xslt/lorem-ipsum-tei.xsl" -s:"$1" -o:"$1.new.xml"
fi
